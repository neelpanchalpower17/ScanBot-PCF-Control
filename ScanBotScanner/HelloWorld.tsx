...
const initialize = async () => {
  const sdk = await ScanbotSDK.initialize({
    ...
    // WASM files are in the wasm folder of the output directory
    enginePath: "./wasm/",
  });
}

const createBarcodeScanner = async () => {
  ...
  barcodeScanner = await ScanbotSDK.instance.createBarcodeScanner(config);
}
...
const reactComp = async () => {
  ...
  return (
    <div>
      ...
      <button onClick={() => void createBarcodeScanner.bind(this)()}>Click me</button>
    </div>
  );
}
